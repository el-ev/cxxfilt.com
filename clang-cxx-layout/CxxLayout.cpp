...
static FieldInfoPtr analyzeRecord(const clang::ASTContext &Ctx,
  ...
  bool IsMSVC = Ctx.getTargetInfo().getCXXABI().isMicrosoft();
  ...
  if (Layout.hasOwnVFPtr()) {
    ...
    VPtrInfo->fieldType = IsMSVC ? FieldType::VFPtr : FieldType::VPtr;
    VPtrInfo->type = IsMSVC ? "VFPtr" : "VPtr";
  ...
  if (Layout.hasOwnVBPtr()) {
    FieldInfoPtr VBPtrInfo = std::make_unique<FieldInfo>();
    VBPtrInfo->isValid = true;
    VBPtrInfo->fieldType = FieldType::VBPtr;
    VBPtrInfo->type = "VBPtr";
    VBPtrInfo->offset = Layout.getVBPtrOffset().getQuantity() * 8;
    VBPtrInfo->size = Ctx.toCharUnitsFromBits(
        Ctx.getTargetInfo().getPointerWidth(clang::LangAS::Default));
    VBPtrInfo->align = Ctx.toCharUnitsFromBits(
        Ctx.getTargetInfo().getPointerAlign(clang::LangAS::Default));
    Info->subFields.push_back(std::move(VBPtrInfo));
  }

  // Virtual bases
  // const clang::ASTRecordLayout::VBaseOffsetsMapTy &VBases =
